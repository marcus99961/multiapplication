<template>
    ...
    <div class="row justify-content-center">
      ...
      <div class="col-md-12">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <button @click="reportTaking"  class="btn-info btn-sm float-center ml-1">Stock Taking</button>
                <button @click="reportSummary"  class="btn-info btn-sm float-center ml-1">Stock Summary</button>
                <button @click="reportBalance"  class="btn-info btn-sm float-center ml-1">Stock Balance</button>
                <button @click="reportLedger"  class="btn-info btn-sm float-center ml-1">Distribution Ledger</button>
                <button @click="stockTransfer"  class="btn-info btn-sm float-right">Stock Transfer</button>
            </div>
        ...
        <div :class="`modal-dialog ${!deleteMode ? 'modal-lg': 'modal-sm'}`">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="supplierModalLabel" v-show="!deleteMode"> {{!editMode ? 'Transfer': 'Update Supplier' }} </h5>
    ...
    <div class="modal fade" id="reportModal" tabindex="-1" aria-labelledby="supplierModalLabel" aria-hidden="true">
        <div :class="`modal-dialog ${!deleteMode ? 'modal-lg': 'modal-sm'}`">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="supplierModalLabel" v-show="!deleteMode"> {{!editMode ? 'Summary by Date': 'Update Supplier' }} </h5>
                    <h5 class="modal-title" id="supplierModalLabel" v-show="deleteMode" > Delete Supplier </h5>
    ...
    <!-- Modal -->
    <div class="modal fade" id="reportbalModal" tabindex="-1" aria-labelledby="supplierModalLabel" aria-hidden="true">
        <div :class="`modal-dialog ${!deleteMode ? 'modal-lg': 'modal-sm'}`">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="supplierModalLabel" v-show="!deleteMode"> {{!ledgerMode ? 'Stock Balance by Location': 'Distribution Ledger by Location' }} </h5>
                    <h5 class="modal-title" id="supplierModalLabel" v-show="deleteMode" > Delete Supplier </h5>
                    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row" v-show="!deleteMode">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <multiselect v-model="reportlocation.location" :options='sourcelocations' :multiple="false" :show-labels="false"
                                :close-on-select="true"> </multiselect>
                                <small class="text-danger" v-if="errors.reportlocation"> {{ errors.reportlocation[0] }} </small><br>
                            </div>
                        </div>
                ...
                <div class="modal-footer" v-show="!deleteMode">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" @click="!ledgerMode ? stockBalancerpt(): ledgerReport()" >{{!ledgerMode ? 'Submit': 'Generate' }}</button>
                </div>
                <div class="modal-footer" v-show="deleteMode">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-danger" @click="deleteSupplier" >Delete</button>
     ...
     <!-- Modal -->
     <div class="modal fade" id="reportModal2" tabindex="-1" aria-labelledby="supplierModalLabel" aria-hidden="true">
        <div :class="`modal-dialog ${!deleteMode ? 'modal-lg': 'modal-sm'}`">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="supplierModalLabel" v-show="!deleteMode"> {{!ledgerMode ? 'Stock Taking Form': 'Stock Taking Form' }} </h5>
                    <h5 class="modal-title" id="supplierModalLabel" v-show="deleteMode" > Delete Supplier </h5>
                    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer" v-show="!deleteMode">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" @click="!ledgerMode ? stockTakingform(): ledgerReport()" >{{!ledgerMode ? 'Submit': 'Generate' }}</button>
                </div>
                <div class="modal-footer" v-show="deleteMode">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-danger" @click="deleteSupplier" >Delete</button>
...
export default {
    ...
    watch: {
        ...
        choseitem : function (value){
            axios.get('/api/getSourceitemqty',{params: { item: this.choseitem, location: this.choselocation }}).then(response=>{
    ...
    this.transferData.qty= '';
        ...
        reportTaking(){
            ...
            axios({
                method:'post',
                url:'/api/getStockbalance',
                responseType:'arraybuffer',
                data: this.reportlocation
                })
                .then(function(response) {
                    let blob = new Blob([response.data], { type:   'application/pdf' } );
                    let link = document.createElement('a');
                    link.href = window.URL.createObjectURL(blob);
                    link.download = 'stock_balance.pdf';
                    link.click();
                    
                });
                $('#reportbalModal').modal('hide')
            },
